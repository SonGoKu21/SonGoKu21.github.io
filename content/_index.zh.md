---
title: "周锦浩个人主页"
---

# 关于我 {#about-me}

我目前是日本早稻田大学信息生产系统研究科（Graduate School of Information, Production and Systems, IPS）的博士研究生，师从 **伍军教授（Prof. Jun Wu）**。我的研究方向主要围绕 **可信人工智能（Trustworthy AI）**，重点关注 **隐私保护机器学习、大语言模型（LLMs）的可靠推理机制以及安全的检索增强生成（RAG）系统**。

我的研究致力于从多个层面构建可信的人工智能系统，包括 **数据隐私保护、分布式模型训练安全以及生成模型推理可靠性评估**。目前已提出多种相关方法，例如 **隐私保护的图注意力学习框架、面向大语言模型的加密检索机制，以及用于幻觉检测的认知可靠性诊断方法**。

相关研究成果已发表在人工智能与信息安全领域的重要期刊与会议上，包括 **IEEE Transactions on Dependable and Secure Computing (TDSC)**、**IEEE Transactions on Information Forensics and Security (TIFS)** 等。

目前正在积极寻找 **博士后或高校教职机会**，研究方向包括可信人工智能、隐私保护机器学习以及可靠的大模型系统。

**研究兴趣包括：**

- 可信人工智能（Trustworthy Artificial Intelligence）  
- 隐私保护机器学习（Privacy-Preserving Machine Learning）  
- 大语言模型（Large Language Models, LLMs）  
- 检索增强生成（Retrieval-Augmented Generation, RAG）  
- 安全可靠的人工智能系统  

---

# 🔥 最新动态 {#news}

- *2026*: 🎉 多项关于隐私保护与可信大模型的研究成果正在投稿至国际期刊与会议。  
- *2025*: 🎉 完成关于安全检索增强生成与大模型幻觉诊断的系列研究。  
- *2024*: 🎉 提出 **CipherRAG** 框架，实现面向大语言模型的隐私保护检索增强生成方法。  

---

# 📝 发表论文 {#publications}

### 期刊论文

- **Efficient Vector-Multiplicative Privacy-Preserving Retrieval-Augmented Generation for Large Language Models**  
  **Jinhao Zhou**, Jun Wu*  
  *IEEE Transactions on Dependable and Secure Computing (TDSC)*, 2026. (CCF-A)  
  [论文链接](https://ieeexplore.ieee.org/document/11419667)

- **DM-DPL: Toward Discrete Matrixing Differentially Private Learning**  
  **Jinhao Zhou**, Zhou Su, Yuntao Wang, Jun Wu* 
  *IEEE Transactions on Information Forensics and Security (TIFS)*, 2025. (CCF-A)  
  [论文链接](https://ieeexplore.ieee.org/document/11037426)

- **Protecting Your Attention during Distributed Graph Learning: Efficient Privacy-Preserving Federated Graph Attention Network**  
  **Jinhao Zhou**, Jun Wu, Jianbing Ni, Yuntao Wang, Yanghe Pan, Zhou Su  
  *IEEE Transactions on Information Forensics and Security (TIFS)*, 2025. (CCF-A)  
  [论文链接](https://ieeexplore.ieee.org/document/10858070)

---

### 会议论文

- **Real-Time Reliable Large Language Models with Distributed Knowledge Crowdsourcing for Automotive Mobile Intelligence**  
  **Jinhao Zhou**, Jun Wu  
  *IEEE International Conference on Wireless Artificial Intelligent Computing Systems and Applications (WASA)*, 2025  
  [论文链接](https://link.springer.com/chapter/10.1007/978-981-96-8725-1_33)

- **Toward Privacy-Aware Efficient Federated Graph Attention Network in Smart Cloud**  
  **Jinhao Zhou**, Zhou Su, Yuntao Wang, Qianqian Pan, Lizheng Liu, Jun Wu  
  *IEEE SmartCloud*, 2024  
  🏆 **Best Paper Award（最佳论文奖）**  
  [论文链接](https://ieeexplore.ieee.org/document/10566319)

- **Personalized Privacy-Preserving Federated Learning: Optimized Trade-off Between Utility and Privacy**  
  **Jinhao Zhou**, Zhou Su, Jianbing Ni, Yuntao Wang, Yanghe Pan, Rui Xing  
  *IEEE Global Communications Conference (GLOBECOM)*, 2022  
  [论文链接](https://ieeexplore.ieee.org/document/10000793)

---

# 🎖 荣誉与奖励 {#honors-and-awards}

- *2024* 🏆 **IEEE SmartCloud 国际会议最佳论文奖（Best Paper Award）**  
- *2019* **国家奖学金**

---

# 📖 教育背景 {#educations}

- *2023 – 2026（预计）*  
  **博士，信息系统**  
  日本早稻田大学（Waseda University）  
  导师：伍军教授（Prof. Jun Wu）

- *2020 – 2023*  
  **硕士，网络空间安全**  
  西安交通大学  
  导师：苏洲教授（Prof. Zhou Su）

- *2016 – 2020*  
  **本科，网络工程**  
  中国地质大学（武汉）